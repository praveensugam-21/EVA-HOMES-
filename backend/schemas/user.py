...
class SellerProfileCreate(BaseModel):
    ...
    # A real photo of the seller — buyers see this on every listing of
    # theirs, so it's mandatory. Upload via POST /me/seller-photo first to
    # get this URL.
    photo_url: str
    ...
    def validate_photo_url(cls, v):
        if not v or not v.strip():
            raise ValueError("A seller photo is required to activate a seller profile.")
        return v
