...
export const authAPI = {
  ...
  /**
   * Upload the seller photo shown to buyers (before or after activating
   * a seller profile). Returns { url }.
   * @param {File} file
   */
  uploadSellerPhoto: async (file) => {
    const formData = new FormData();
    formData.append("file", file);
    const response = await api.post("/api/auth/me/seller-photo", formData, {
