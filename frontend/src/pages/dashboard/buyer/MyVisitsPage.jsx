          ...
          {items.map((visit) => (
            <article key={visit.id} className="bg-white border border-zinc-200 rounded-xl shadow-sm p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/properties/${visit.property_id}`} className="font-semibold text-zinc-900 hover:underline">
                  {visit.property_title}
                </Link>
                <p className="text-sm text-zinc-600 mt-1">Requested: {new Date(visit.requested_date).toLocaleString()}</p>
